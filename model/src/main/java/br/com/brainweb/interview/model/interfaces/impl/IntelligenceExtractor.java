...
public class IntelligenceExtractor implements HeroStatsExtractor {

    @Override
    public List<HeroSkill> extractStats(List<HeroDTO> heroes) {
        List<HeroSkill> stats = new ArrayList<>();
        heroes.forEach(hero -> stats.add(new HeroSkill(hero.getName(), hero.getPowerStats().getIntelligence())));
        return stats;
    }
}